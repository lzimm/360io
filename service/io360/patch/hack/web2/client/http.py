# -*- test-case-name: hack.web2.test.test_client -*-
...
from hack.web2.responsecode import BAD_REQUEST, HTTP_VERSION_NOT_SUPPORTED
from hack.web2.http import parseVersion, Response
from hack.web2.http_headers import Headers
from hack.web2.stream import ProducerStream, StreamProducer, IByteStream
from hack.web2.channel.http import HTTPParser, PERSIST_NO_PIPELINE, PERSIST_PIPELINE
from hack.web2.client.interfaces import IHTTPClientManager
...
class ClientRequest(object):
    ...
    def __init__(self, method, uri, headers, stream):
        ...
        @type headers: C{dict}, L{hack.web2.http_headers.Headers}, or
            C{None}

        @param stream: Content body to send to the remote HTTP server.
        @type stream: L{hack.web2.stream.IByteStream}
        """
...
class HTTPClientProtocol(LineReceiver, TimeoutMixin, object):
    ...
    def submitRequest(self, request, closeAfter=True):
        ...
        @return: A Deferred which will be called back with the
            L{hack.web2.http.Response} from the server.
        """
