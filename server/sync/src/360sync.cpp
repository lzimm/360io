...
void nodeAccept(ev::io &w, int revents) {
    ...
    SyncClusterConnection* conn = new SyncClusterConnection(client_fd);
    conn->start();
}
...
void httpAccept(ev::io &w, int revents) {
    ...
    SyncRequest* req = new SyncRequest(client_fd);
    req->start();
}
...
void setupNode(ev::io& node) {
    ...
    node_addr.sin_port              = htons(SYNC_NODE_PORT);
...
void setupHTTP(ev::io& http) {
    ...
    http_addr.sin_port              = htons(SYNC_HTTP_PORT);
