...
void nodeAccept(ev::io &w, int revents) {
    ...
    LogClusterConnection* conn = new LogClusterConnection(client_fd);
    conn->start();
}
...
void httpAccept(ev::io &w, int revents) {
    ...
    LogRequest* req = new LogRequest(client_fd);
    req->start();
}
...
void setupNode(ev::io& node) {
    ...
    node_addr.sin_port              = htons(LOG_NODE_PORT);
...
void setupHTTP(ev::io& http) {
    ...
    http_addr.sin_port              = htons(LOG_HTTP_PORT);
